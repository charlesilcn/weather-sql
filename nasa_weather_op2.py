...
# -*- coding: utf-8 -*-
"""
NASA POWER Optimized v2 · 多年分段重试
Segmented + Retry + Resume · 2020-2024 full history
"""
import io
import pandas as pd
import requests
...
OUT_DIR = 'nasa_history_op2'             # 独立目录，不影响 op
...
# ===== 单年分段抓取 =====
def fetch_segments(city_id, name, lat, lng, year):
...
def main():
    ...
    print('Step 4-B：NASA POWER Optimized v2 · 多年分段重试 2020-2024')
    all_df = []
    for year in YEARS:
        print(f'\n===== Year {year} =====')
        for cid, (name, lat, lng) in tqdm(CITIES.items(), desc=f'{year} Cities'):
            csv_txt = fetch_segments(cid, name, lat, lng, year)
            if not csv_txt:
                continue
    ...
    all_csv = os.path.abspath('all_history_op2.csv')
...
def load2mysql(csv_path):
    ...
    with pymysql.connect(**DB) as conn:
        with conn.cursor() as cur:
            ...
            sql = f"""
            LOAD DATA LOCAL INFILE '{csv_unix}'
